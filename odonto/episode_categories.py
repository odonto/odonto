...
class CovidTriageEpisode(episodes.EpisodeCategory, AbstractOdontoCategory):
    ...
    def get_submit_link(self):
        patient_id = self.episode.patient_id
        episode_id = self.episode.id
        return f"{settings.HOST_NAME_AND_PROTOCOL}/pathway/#/covid-triage-submit/{patient_id}/{episode_id}"

    def get_edit_link(self):
        patient_id = self.episode.patient_id
        episode_id = self.episode.id
        return f"{settings.HOST_NAME_AND_PROTOCOL}/pathway/#/covid-triage-edit/{patient_id}/{episode_id}"
